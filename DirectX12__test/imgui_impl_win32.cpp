...
static void ImGui_ImplWin32_DestroyWindow(ImGuiViewport* viewport)
{
    ...
    if (ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData)
    {
        ...
        if (vd->Hwnd && vd->HwndOwned)
        {
            ...
            // IME/TSF関連のクリーンアップを試行（例外が発生する可能性がある）
            __try
            {
                // IMEコンテキストの切断を試行
                ...
                if (hImc)
                {
                    ::ImmReleaseContext(vd->Hwnd, hImc);
                }
                ::ImmAssociateContextEx(vd->Hwnd, NULL, 0);
            }
            ...
            // DestroyWindowも例外が発生する可能性があるため保護
            __try
            {
            ...
            __except (EXCEPTION_EXECUTE_HANDLER)
            {
                // msctf.dllの既知の問題で例外が発生する場合がある
                // ウィンドウハンドルは無効になっているので続行
            }
        }
        vd->Hwnd = nullptr;
        IM_DELETE(vd);
