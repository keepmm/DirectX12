...
void EditorWindow::Draw(SceneManager& sceneManager)
{
	...
	// ---- ビューポートパネル ---- //
	const bool GameViewOpen = ImGui::Begin(u8("ゲーム画面"));
	m_GameViewVisible = GameViewOpen && m_ShowViewport;
	if (GameViewOpen && m_ShowViewport)
	{
	...
	// ---- エディタ画面パネル ---- //
	const bool EditorViewOpen = ImGui::Begin(u8("エディタ画面"));
	m_EditorViewVisible = EditorViewOpen && m_ShowViewport;
	if (EditorViewOpen && m_ShowViewport)
	{
		ImVec2 availableSize = ImGui::GetContentRegionAvail();
		if (ImGui::IsWindowHovered())
			INPUT->SetViewportHovered(true);
	...
	if (ImGui::Begin(u8("プロファイラ")) && m_ShowProfiler)
	{
		DrawProfiler();
	}
	ImGui::End();
...
#include "Theme.hpp"
void EditorWindow::DrawProfiler()
{
